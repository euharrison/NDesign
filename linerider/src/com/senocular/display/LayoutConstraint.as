	...
	public class LayoutConstraint extends EventDispatcher {
		...
		public function setIn(container:Rectangle):void {
			...
			if (container){
				if (!alignedLeft && !alignedRight) {
					if (noHorizontalCenter && noPercentHorizontalCenter) { // normal
						if(_width) _rect.width = isNaN(_percentWidth) ? _width : _percentWidth*container.width;
						...
						if (noPercentHorizontalCenter) {
							...
							_rect.x = currValue - (_rect.width || 0)/2 + container.left;
						}
					}
					...
					if(_width) _rect.width = isNaN(_percentWidth) ? _width : _percentWidth*container.width;
					_rect.x = noPercentRight ? container.right - _right - (_rect.width || 0) : container.right - _percentRight*container.width - (_rect.width || 0);
			...
			if (currValue){ // if change in width, adjust position
				...
				// fit width
				if(_width) _rect.width -= currValue;
			}
			...
			if (container){
				if (!alignedTop && !alignedBottom) {
					...
					if (noVerticalCenter && noPercentVerticalCenter) { // normal
						if(_height) _rect.height = isNaN(_percentHeight) ? _height : _percentHeight*container.height;
			...
			if (currValue){ // if change in height, adjust position
				...
				// fit height
				if(_height) _rect.height -= currValue;
			}
